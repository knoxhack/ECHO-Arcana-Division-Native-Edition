...
const manifestPath = path.join(dist, 'arcana-division-native-edition-beta-1.0.0.pack.json')
...
if (release.releaseTag !== 'arcana-division-native-1.0.0-beta') errors.push('release tag mismatch')
...
if (errors.length) {
  console.error('Arcana Division Native Edition validation failed:')
  for (const error of errors) console.error(`- ${error}`)
  process.exit(1)
}

console.log('Arcana Division Native Edition validation passed.')
